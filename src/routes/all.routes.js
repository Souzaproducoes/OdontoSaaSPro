// =============================================
// ALL.ROUTES.JS — CONSOLIDADO E CORRIGIDO
// =============================================
...
financeiroRouter.get('/', async (req, res) => {
  try {
    ...
    if (mes) {
      ...
      const proximoMes = String(Number(m) + 1).padStart(2, '0');
      query = query
        .gte('created_at', `${ano}-${m}-01`)
        .lt('created_at', `${ano}-${proximoMes}-01`);
    }
...
tenantRouter.patch('/me', adminOnly, async (req, res) => {
  try {
    const camposPermitidos = [
      'nome_clinica', 'telefone', 'endereco', 'horario_funcionamento',
      'especialidades', 'convenios', 'tabela_precos', 
      'condicoes_pagamento', 'email', 'site'
    ];
    const updates = {};
